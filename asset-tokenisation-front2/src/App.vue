<template>
  <div>
    ...
    <div id="app" class="container test rounded pb-3">
      ...
      <div v-show="!logged" class="mt-5">
        <h2 class="pt-3 ml-5 mb-5">Connection</h2>
        
          <form v-on:submit.stop.prevent="authenfication()" id="" class="was-validated" novalidate>
                    <div class="form-group row mb-4">
                      <div class="offset-md-3"></div>
                        <label for="public_key" class="col-sm-2 col-form-label">User Id's</label>
                        <div class="col-sm-4">
                            <select class="form-control" name="user_id" v-model="selected_user_id" required>
                                <option v-for="user_id in users_id" v-bind:value="user_id">
                                    {{user_id}}
                                </option>
                            </select>
                    ...
                    <div class="row no-gutters">
                      ...
                      <button type="submit" class="btn btn-secondary btn-wl btn-block col-5">
                          Connect 
                      </button>
                    </div>
                </form>
...
export default {
  ...
  created: function () {
    // retrieve localhost providers from truffle 
  ...
  methods: {
    ...
    startApp() {
      ...
      var contractAddress = '0x723eeb2cb47dab00c3c4af847e7d6abb965c4570';
      this.cryptoBarrelsContract = web3.eth.contract(bidABI).at(contractAddress);
      this.balance = web3.eth.getBalance(this.userAddress);
      this.logged = true;
    },
...
body {
  background: #ffffff url("../static/background1.jpg") no-repeat right top;
